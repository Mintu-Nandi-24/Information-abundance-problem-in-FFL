# =============================================================================
# FILE: Numerics/Gillespie-after-optimization-IMI/Ecoli/OL/AND/ecoli-ol-c1-and.py
...
#
# PURPOSE
#   Runs a Gillespie simulation of the C1 matched open-loop (OL) reference in E. coli with an AND output gate and estimates I(X, X_tilde; Z).
...
#
# MODEL CONDITION
#   Motif: C1; edge signs (X->Y, X->Z, Y->Z) = (+,+,+).
#   X->Y: activation; X->Z: activation; Y->Z: activation; gate: AND.
#   Simulation time: 5000000.
#   Hard-coded parameter set: <x1>,<x2>,<y>,<z>=(10.64,10.64,13.17,99.94); beta=(0.0050007,0.0050007,0.099884,0.49986); K=(63.52,69.95,18.4).
...
#   python OL/AND/ecoli-ol-c1-and.py
#
# OUTPUT
#   total-MI-abund-c1ol-and-num.dat in the current working directory.
...
x1 = int(round(x1av_param))
x2 = int(round(x2av_param))
y = int(round(yav_param))
z = int(round(zav_param))
...
while time < simulation_time:
    ...
    # Time-averaged update before the reaction event
    dt_sample = min(delta_t, simulation_time - time)

    tot_x1 += x1 * dt_sample
    tot_x2 += x2 * dt_sample
    tot_z += z * dt_sample
    
    tot_x1_2 += x1*x1 * dt_sample
    tot_x2_2 += x2*x2 * dt_sample
    tot_z2 += z*z * dt_sample
    
    tot_x1z += x1*z * dt_sample
    tot_x2z += x2*z * dt_sample

    sampled_time += dt_sample
...
x1av = tot_x1/sampled_time
x2av = tot_x2/sampled_time
zav = tot_z/sampled_time

x1var = (tot_x1_2/sampled_time) - (x1av*x1av)
x2var = (tot_x2_2/sampled_time) - (x2av*x2av)
zvar = (tot_z2/sampled_time) - (zav*zav)

x1zcov = (tot_x1z/sampled_time) - (x1av*zav)
x2zcov = (tot_x2z/sampled_time) - (x2av*zav)
...
with open('total-MI-abund-c1ol-and-num.dat', 'w') as f:
