# =============================================================================
# FILE: Numerics/CEM-optimization-IMI/imi-coherent-yeast-or.py
...
#   ($I_{\mathrm{int}}(X;Z)$) for coherent FFLs in *S. cerevisiae* / yeast with an OR
#   output gate. The four motifs (C1, C2, C3, C4) share one nine-parameter
#   operating-point vector in each candidate evaluation.
#
# OPTIMIZATION SETUP
#   Target windows: C1/C2 in [2, 9], C1/C3 in [10, 150], C1/C4 in [10, 150].
...
#   Run: python imi-coherent-yeast-or.py
#
# OUTPUTS
#   Relative output directory: coherent_yeast_OR_data/
#   Files: coherent-yeast-OR-diagnostics.dat, coherent-yeast-OR-best_penalty_convergence.dat, coherent-yeast-OR-imi_trajectory.dat, coherent-yeast-OR-final_imi_summary.dat, coherent-yeast-OR-ratio_satisfaction_scatter.dat, coherent-yeast-OR-avg_imi_by_run.dat, coherent-yeast-OR-pairwise_distances.dat, coherent-yeast-OR-parameter_correlations.dat, coherent-yeast-OR-normalized_sensitivity.dat, coherent-yeast-OR-parameter_cv.dat, coherent-yeast-OR-violin_data.dat, coherent-yeast-OR-final_parameter_summary.dat
...
EXPORT_DIR = "coherent_yeast_OR_data"
...
def export_all_data(multi_results: Dict[str, Any]):
    ...
    for i in range(multi_results['n_iter']):
        ...
        data1.append(row)
    export_to_dat(
        "coherent-yeast-OR-diagnostics.dat",
    ...
    data2 = [[iterations[i], multi_results['best_penalty_mean'][i], multi_results['best_penalty_std'][i]]
             for i in range(multi_results['n_iter'])]
    export_to_dat(
        "coherent-yeast-OR-best_penalty_convergence.dat",
    ...
    for i in range(multi_results['n_iter']):
        ...
        data3.append(row)
    export_to_dat(
        "coherent-yeast-OR-imi_trajectory.dat",
    ...
    export_to_dat(
        "coherent-yeast-OR-final_imi_summary.dat",
    ...
    export_to_dat(
        "coherent-yeast-OR-ratio_satisfaction_scatter.dat",
    ...
    export_to_dat(
        "coherent-yeast-OR-avg_imi_by_run.dat",
    ...
    if dist_matrix.size > 0:
        ...
        export_to_dat(
            "coherent-yeast-OR-pairwise_distances.dat",
            dist_matrix,
            header7,
    ...
    export_to_dat(
        "coherent-yeast-OR-parameter_correlations.dat",
        corr_matrix,
        PARAM_NAMES,
    ...
    for p in PARAM_NAMES:
        ...
        data9.append(row)
    export_to_dat(
        "coherent-yeast-OR-normalized_sensitivity.dat",
    ...
    export_to_dat(
        "coherent-yeast-OR-parameter_cv.dat",
    ...
    export_to_dat(
        "coherent-yeast-OR-violin_data.dat",
    ...
    data12 = [[PARAM_NAMES[i], multi_results['theta_means'][i], multi_results['theta_stds'][i]]
              for i in range(len(PARAM_NAMES))]
    export_to_dat(
        "coherent-yeast-OR-final_parameter_summary.dat",
