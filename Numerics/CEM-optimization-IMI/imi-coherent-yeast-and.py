# =============================================================================
# FILE: Numerics/CEM-optimization-IMI/imi-coherent-yeast-and.py
...
#   ($I_{\mathrm{int}}(X;Z)$) for coherent FFLs in *S. cerevisiae* / yeast with an AND
#   output gate. The four motifs (C1, C2, C3, C4) share one nine-parameter
#   operating-point vector in each candidate evaluation.
#
# OPTIMIZATION SETUP
#   Target windows: C1/C2 in [3, 8], C1/C3 in [20, 150], C1/C4 in [20, 150].
...
#   Run: python imi-coherent-yeast-and.py
#
# OUTPUTS
#   Relative output directory: coherent_yeast_AND_data/
#   Files: coherent-yeast-AND-diagnostics.dat, coherent-yeast-AND-best_penalty_convergence.dat, coherent-yeast-AND-imi_trajectory.dat, coherent-yeast-AND-final_imi_summary.dat, coherent-yeast-AND-ratio_satisfaction_scatter.dat, coherent-yeast-AND-avg_imi_by_run.dat, coherent-yeast-AND-pairwise_distances.dat, coherent-yeast-AND-parameter_correlations.dat, coherent-yeast-AND-normalized_sensitivity.dat, coherent-yeast-AND-parameter_cv.dat, coherent-yeast-AND-violin_data.dat, coherent-yeast-AND-final_parameter_summary.dat
...
EXPORT_DIR = "coherent_yeast_AND_data"
...
def export_all_data(multi_results: Dict[str, Any]):
    ...
    for i in range(multi_results['n_iter']):
        ...
        data1.append(row)
    export_to_dat(
        "coherent-yeast-AND-diagnostics.dat",
    ...
    data2 = [[iterations[i], multi_results['best_penalty_mean'][i], multi_results['best_penalty_std'][i]]
             for i in range(multi_results['n_iter'])]
    export_to_dat(
        "coherent-yeast-AND-best_penalty_convergence.dat",
    ...
    for i in range(multi_results['n_iter']):
        ...
        data3.append(row)
    export_to_dat(
        "coherent-yeast-AND-imi_trajectory.dat",
    ...
    export_to_dat(
        "coherent-yeast-AND-final_imi_summary.dat",
    ...
    export_to_dat(
        "coherent-yeast-AND-ratio_satisfaction_scatter.dat",
    ...
    export_to_dat(
        "coherent-yeast-AND-avg_imi_by_run.dat",
    ...
    if dist_matrix.size > 0:
        ...
        export_to_dat(
            "coherent-yeast-AND-pairwise_distances.dat",
            dist_matrix,
            header7,
    ...
    export_to_dat(
        "coherent-yeast-AND-parameter_correlations.dat",
        corr_matrix,
        PARAM_NAMES,
    ...
    for p in PARAM_NAMES:
        ...
        data9.append(row)
    export_to_dat(
        "coherent-yeast-AND-normalized_sensitivity.dat",
    ...
    export_to_dat(
        "coherent-yeast-AND-parameter_cv.dat",
    ...
    export_to_dat(
        "coherent-yeast-AND-violin_data.dat",
    ...
    data12 = [[PARAM_NAMES[i], multi_results['theta_means'][i], multi_results['theta_stds'][i]]
              for i in range(len(PARAM_NAMES))]
    export_to_dat(
        "coherent-yeast-AND-final_parameter_summary.dat",
