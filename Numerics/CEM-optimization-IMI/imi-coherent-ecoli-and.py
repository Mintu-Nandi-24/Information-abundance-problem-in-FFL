# =============================================================================
# FILE: Numerics/CEM-optimization-IMI/imi-coherent-ecoli-and.py
...
#
# PURPOSE
#   Performs abundance-guided cross-entropy optimization of interference mutual information
#   ($I_{\mathrm{int}}(X;Z)$) for coherent FFLs in *E. coli* with an AND
#   output gate. The four motifs (C1, C2, C3, C4) share one nine-parameter
#   operating-point vector in each candidate evaluation.
#
# OPTIMIZATION SETUP
#   Target windows: C1/C2 in [8, 10], C1/C3 in [2, 3], C1/C4 in [4, 5].
...
#   Recommended working directory: Numerics/CEM-optimization-IMI/
#   Run: python imi-coherent-ecoli-and.py
#
# OUTPUTS
#   Relative output directory: coherent_ecoli_AND_data/
#   Files: coherent-ecoli-AND-diagnostics.dat, coherent-ecoli-AND-best_penalty_convergence.dat, coherent-ecoli-AND-imi_trajectory.dat, coherent-ecoli-AND-final_imi_summary.dat, coherent-ecoli-AND-ratio_satisfaction_scatter.dat, coherent-ecoli-AND-avg_imi_by_run.dat, coherent-ecoli-AND-pairwise_distances.dat, coherent-ecoli-AND-parameter_correlations.dat, coherent-ecoli-AND-normalized_sensitivity.dat, coherent-ecoli-AND-parameter_cv.dat, coherent-ecoli-AND-violin_data.dat, coherent-ecoli-AND-final_parameter_summary.dat
...
#   ixzs and the returned motif values correspond to I_int(X;Z).
# =============================================================================
...
EXPORT_DIR = "coherent_ecoli_AND_data"
...
MOTIF_NAMES = list(COHERENT_MOTIFS.keys())

PARAM_NAMES = ["beta_x", "beta_y", "beta_z", "Kxy", "Kxz", "Kyz", "xbar", "ybar", "zbar"]

# Desired ordering windows for coherent FFLs
CFFL_TARGETS = {
    ("C1", "C2"): (8.0, 10.0),
    ("C1", "C3"): (2.0, 3.0),
    ("C1", "C4"): (4.0, 5.0),
...
# Parameter bounds
LO_THETA = np.array([5e-3, 5e-3, 5e-3, 5.0, 5.0, 5.0, 10.0, 10.0, 10.0], float)
HI_THETA = np.array([1e-1, 1e-1, 5e-1, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0], float)
...
# ============================================================
# 3) ANALYTICAL IMI CALCULATION
# ============================================================
def imi_analytical_cffl(theta: np.ndarray, motif: str) -> Tuple[float, bool]:
    ...
    s_xy, s_xz, s_yz = COHERENT_MOTIFS[motif]
    ...
    fzxp = alpha_z * fzy * hill_prime(xbar, Kxz, s_xz)
    fzyp = alpha_z * fzx * hill_prime(ybar, Kyz, s_yz)
    ...
    return float(ixzs), True


def calculate_all_cffl_imis(theta: np.ndarray) -> Tuple[Dict[str, float], bool]:
    imis: Dict[str, float] = {}
    ok = True
    for motif in COHERENT_MOTIFS:
        value, valid = imi_analytical_cffl(theta, motif)
        imis[motif] = value
        ok = ok and valid and np.isfinite(value)
    return imis, ok


def compute_cffl_ratios(imis: Dict[str, float]) -> Dict[str, float]:
    ratios: Dict[str, float] = {}
    for (a, b), _ in CFFL_TARGETS.items():
        ratios[f"{a}/{b}"] = imis[a] / imis[b]
    return ratios


def ratio_penalty_from_imis(imis: Dict[str, float]) -> float:
    penalty = 0.0
    for (a, b), (lo, hi) in CFFL_TARGETS.items():
        r = imis[a] / imis[b]
        if r <= 0:
            return float("inf")
        lr = math.log(r)
...
def evaluate_theta(theta: np.ndarray) -> Dict[str, Any]:
    """
    Objective:
      1) Preserve required ratio ordering (feasibility first)
      2) Among feasible solutions, maximize the average IMI magnitude
    """
    imis, ok = calculate_all_cffl_imis(theta)
    ...
    if any(imis[m] <= 0 for m in COHERENT_MOTIFS):
        return {
            'loss': INFEASIBLE_OFFSET + 1e3,
            'penalty': float('inf'),
            'avg_imi': -float('inf'),
    ...
    ratios = compute_cffl_ratios(imis)
    penalty = ratio_penalty_from_imis(imis)
    avg_imi = float(np.mean([imis[m] for m in COHERENT_MOTIFS]))
...
def normalized_sensitivity_analysis(
    ...
    """
    original_imis, ok = calculate_all_cffl_imis(theta)
    if not ok:
    ...
    results = {
        ...
        'motif_sensitivity': {motif: {p: 0.0 for p in param_names} for motif in COHERENT_MOTIFS},
    }
    ...
    for i, param_name in enumerate(param_names):
        ...
        for motif in COHERENT_MOTIFS:
            ...
            for perturbation in np.linspace(-perturbation_frac, perturbation_frac, n_perturbations):
                ...
                perturbed_imis, ok = calculate_all_cffl_imis(perturbed_theta)
                if not ok:
                    continue
        ...
        across = [results['motif_sensitivity'][m][param_name] for m in COHERENT_MOTIFS]
...
def run_multiple_optimizations(n_runs: int = N_RUNS, n_samples: int = N_SAMPLES_FEASIBILITY) -> Dict[str, Any]:
    print("=" * 80)
    print(f"MULTI-RUN CFFL OPTIMIZATION ({n_runs} runs)")
    print("Objective: preserve ordering, then maximize average IMI")
    print("=" * 80)
    ...
    all_ratios_by_run = {
        ...
        for (a, b) in CFFL_TARGETS
    }

    imi_means = {m: float(np.nanmean([r.best_imis.get(m, np.nan) for r in successful_results])) for m in COHERENT_MOTIFS}
    imi_stds = {m: float(np.nanstd([r.best_imis.get(m, np.nan) for r in successful_results], ddof=1)) for m in COHERENT_MOTIFS}
...
def export_all_data(multi_results: Dict[str, Any]):
    ...
    for i in range(multi_results['n_iter']):
        ...
        data1.append(row)
    export_to_dat(
        "coherent-ecoli-AND-diagnostics.dat",
    ...
    data2 = [[iterations[i], multi_results['best_penalty_mean'][i], multi_results['best_penalty_std'][i]]
             for i in range(multi_results['n_iter'])]
    export_to_dat(
        "coherent-ecoli-AND-best_penalty_convergence.dat",
    ...
    for i in range(multi_results['n_iter']):
        ...
        data3.append(row)
    export_to_dat(
        "coherent-ecoli-AND-imi_trajectory.dat",
    ...
    export_to_dat(
        "coherent-ecoli-AND-final_imi_summary.dat",
    ...
    ratio_keys = [f"{a}/{b}" for (a, b) in CFFL_TARGETS]
    data5 = []
    ...
    export_to_dat(
        "coherent-ecoli-AND-ratio_satisfaction_scatter.dat",
    ...
    export_to_dat(
        "coherent-ecoli-AND-avg_imi_by_run.dat",
    ...
    if dist_matrix.size > 0:
        ...
        export_to_dat(
            "coherent-ecoli-AND-pairwise_distances.dat",
            dist_matrix,
            header7,
    ...
    export_to_dat(
        "coherent-ecoli-AND-parameter_correlations.dat",
        corr_matrix,
        PARAM_NAMES,
    ...
    for p in PARAM_NAMES:
        ...
        data9.append(row)
    export_to_dat(
        "coherent-ecoli-AND-normalized_sensitivity.dat",
    ...
    export_to_dat(
        "coherent-ecoli-AND-parameter_cv.dat",
    ...
    export_to_dat(
        "coherent-ecoli-AND-violin_data.dat",
    ...
    data12 = [[PARAM_NAMES[i], multi_results['theta_means'][i], multi_results['theta_stds'][i]]
              for i in range(len(PARAM_NAMES))]
    export_to_dat(
        "coherent-ecoli-AND-final_parameter_summary.dat",
...
def plot_diagnostics_figure(multi_results: Dict[str, Any]):
    ...
    ax_avg.set_ylabel('Best average IMI so far')
...
def plot_final_imi_bar(multi_results: Dict[str, Any]):
    ...
    ax.set_title('Final IMI of Each FFL Type', fontweight='bold')
...
# 10) MAIN EXECUTION
# ============================================================
def run_cffl_multi_optimization(n_runs: int = N_RUNS, n_samples: int = N_SAMPLES_FEASIBILITY):
...
if __name__ == "__main__":
    results = run_cffl_multi_optimization()
