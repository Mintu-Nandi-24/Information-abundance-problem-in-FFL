# =============================================================================
# FILE: Numerics/CEM-optimization-IMI/imi-incoherent-ecoli-and.py
...
#   ($I_{\mathrm{int}}(X;Z)$) for incoherent FFLs in *E. coli* with an AND
#   output gate. The four motifs (I1, I2, I3, I4) share one nine-parameter
#   operating-point vector in each candidate evaluation.
#
# OPTIMIZATION SETUP
#   Target windows: I1/I2 in [8, 10], I1/I3 in [3.5, 4.5], I1/I4 in [4.5, 5.5].
...
#   Run: python imi-incoherent-ecoli-and.py
#
# OUTPUTS
#   Relative output directory: incoherent_ecoli_AND_data/
#   Files: incoherent-ecoli-AND-diagnostics.dat, incoherent-ecoli-AND-best_penalty_convergence.dat, incoherent-ecoli-AND-imi_trajectory.dat, incoherent-ecoli-AND-final_imi_summary.dat, incoherent-ecoli-AND-ratio_satisfaction_scatter.dat, incoherent-ecoli-AND-avg_imi_by_run.dat, incoherent-ecoli-AND-pairwise_distances.dat, incoherent-ecoli-AND-parameter_correlations.dat, incoherent-ecoli-AND-normalized_sensitivity.dat, incoherent-ecoli-AND-parameter_cv.dat, incoherent-ecoli-AND-violin_data.dat, incoherent-ecoli-AND-final_parameter_summary.dat
...
EXPORT_DIR = "incoherent_ecoli_AND_data"
...
def compute_iffl_ratios(imis: Dict[str, float]) -> Dict[str, float]:
    ...
    for (a, b), _ in IFFL_TARGETS.items():
        ratios[f"|{a}|/|{b}|"] = abs(imis[a]) / abs(imis[b])
    return ratios
...
def evaluate_theta(theta: np.ndarray) -> Dict[str, Any]:
    ...
    # Incoherent constraint: all IMI must be NEGATIVE
    if any(imis[m] >= 0 for m in INCOHERENT_MOTIFS):
        return {
            'loss': INFEASIBLE_OFFSET + 1e3,
            'penalty': float('inf'),
            'avg_imi': -float('inf'),
...
def export_all_data(multi_results: Dict[str, Any]):
    ...
    for i in range(multi_results['n_iter']):
        ...
        data1.append(row)
    export_to_dat(
        "incoherent-ecoli-AND-diagnostics.dat",
    ...
    data2 = [[iterations[i], multi_results['best_penalty_mean'][i], multi_results['best_penalty_std'][i]]
             for i in range(multi_results['n_iter'])]
    export_to_dat(
        "incoherent-ecoli-AND-best_penalty_convergence.dat",
    ...
    for i in range(multi_results['n_iter']):
        ...
        data3.append(row)
    export_to_dat(
        "incoherent-ecoli-AND-imi_trajectory.dat",
    ...
    export_to_dat(
        "incoherent-ecoli-AND-final_imi_summary.dat",
    ...
    export_to_dat(
        "incoherent-ecoli-AND-ratio_satisfaction_scatter.dat",
    ...
    export_to_dat(
        "incoherent-ecoli-AND-avg_imi_by_run.dat",
    ...
    if dist_matrix.size > 0:
        ...
        export_to_dat(
            "incoherent-ecoli-AND-pairwise_distances.dat",
            dist_matrix,
            header7,
    ...
    export_to_dat(
        "incoherent-ecoli-AND-parameter_correlations.dat",
        corr_matrix,
        PARAM_NAMES,
    ...
    for p in PARAM_NAMES:
        ...
        data9.append(row)
    export_to_dat(
        "incoherent-ecoli-AND-normalized_sensitivity.dat",
    ...
    export_to_dat(
        "incoherent-ecoli-AND-parameter_cv.dat",
    ...
    export_to_dat(
        "incoherent-ecoli-AND-violin_data.dat",
    ...
    data12 = [[PARAM_NAMES[i], multi_results['theta_means'][i], multi_results['theta_stds'][i]]
              for i in range(len(PARAM_NAMES))]
    export_to_dat(
        "incoherent-ecoli-AND-final_parameter_summary.dat",
