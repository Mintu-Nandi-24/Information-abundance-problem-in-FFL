# =============================================================================
# FILE: Numerics/CEM-optimization-IMI/imi-incoherent-yeast-and.py
...
#   ($I_{\mathrm{int}}(X;Z)$) for incoherent FFLs in *S. cerevisiae* / yeast with an AND
...
#   Run: python imi-incoherent-yeast-and.py
#
# OUTPUTS
#   Relative output directory: incoherent_yeast_AND_data/
#   Files: incoherent-yeast-AND-diagnostics.dat, incoherent-yeast-AND-best_penalty_convergence.dat, incoherent-yeast-AND-imi_trajectory.dat, incoherent-yeast-AND-final_imi_summary.dat, incoherent-yeast-AND-ratio_satisfaction_scatter.dat, incoherent-yeast-AND-avg_imi_by_run.dat, incoherent-yeast-AND-pairwise_distances.dat, incoherent-yeast-AND-parameter_correlations.dat, incoherent-yeast-AND-normalized_sensitivity.dat, incoherent-yeast-AND-parameter_cv.dat, incoherent-yeast-AND-violin_data.dat, incoherent-yeast-AND-final_parameter_summary.dat
...
EXPORT_DIR = "incoherent_yeast_AND_data"
...
def export_all_data(multi_results: Dict[str, Any]):
    ...
    for i in range(multi_results['n_iter']):
        ...
        data1.append(row)
    export_to_dat(
        "incoherent-yeast-AND-diagnostics.dat",
    ...
    data2 = [[iterations[i], multi_results['best_penalty_mean'][i], multi_results['best_penalty_std'][i]]
             for i in range(multi_results['n_iter'])]
    export_to_dat(
        "incoherent-yeast-AND-best_penalty_convergence.dat",
    ...
    for i in range(multi_results['n_iter']):
        ...
        data3.append(row)
    export_to_dat(
        "incoherent-yeast-AND-imi_trajectory.dat",
    ...
    export_to_dat(
        "incoherent-yeast-AND-final_imi_summary.dat",
    ...
    export_to_dat(
        "incoherent-yeast-AND-ratio_satisfaction_scatter.dat",
    ...
    export_to_dat(
        "incoherent-yeast-AND-avg_imi_by_run.dat",
    ...
    if dist_matrix.size > 0:
        ...
        export_to_dat(
            "incoherent-yeast-AND-pairwise_distances.dat",
            dist_matrix,
            header7,
    ...
    export_to_dat(
        "incoherent-yeast-AND-parameter_correlations.dat",
        corr_matrix,
        PARAM_NAMES,
    ...
    for p in PARAM_NAMES:
        ...
        data9.append(row)
    export_to_dat(
        "incoherent-yeast-AND-normalized_sensitivity.dat",
    ...
    export_to_dat(
        "incoherent-yeast-AND-parameter_cv.dat",
    ...
    export_to_dat(
        "incoherent-yeast-AND-violin_data.dat",
    ...
    data12 = [[PARAM_NAMES[i], multi_results['theta_means'][i], multi_results['theta_stds'][i]]
              for i in range(len(PARAM_NAMES))]
    export_to_dat(
        "incoherent-yeast-AND-final_parameter_summary.dat",
